...
async function decryptWhatsAppImage(imageData) {
  ...
  try {
    ...
    if (plaintext_hash) {
      const plainSha = createHash('sha256').update(decrypted).digest('base64');
      if (plainSha !== plaintext_hash) {
        console.warn('Plaintext hash mismatch');
      }
    }
...
async function uploadGeneratedImageToSupabase(base64Data, mimeType) {
  if (!CONFIG.supabase.url || !CONFIG.supabase.s3Endpoint || !CONFIG.supabase.accessKeyId) {
    throw new Error('Missing Supabase S3 configuration');
  }
  ...
  const s3 = new S3Client({
    region: CONFIG.supabase.s3Region,
    endpoint: CONFIG.supabase.s3Endpoint,
    credentials: {
  ...
  const publicUrl = `${CONFIG.supabase.url.replace(/\/+$/, '')}/storage/v1/object/public/${encodeURIComponent(CONFIG.supabase.bucket)}/${encodeURIComponent(filename)}`;
  console.log('Generated image uploaded:', publicUrl);
  return publicUrl;
}

async function sendWhatsAppImageMessage(toE164, imageUrl, caption) {
  if (!toE164) throw new Error('Missing recipient phone number');
  if (!imageUrl) throw new Error('Missing image URL');
  ...
  if (action === 'data_exchange') {
    if (data && typeof data === 'object') {
      ...
      try {
        ...
        return {
          screen: 'COLLECT_IMAGE_SCENE',
          data: { error_message: `Failed to process image: ${imageError.message}` }
        };
      }
      ...
      generateImageAndSendToUser(
        ...
        actualImageData,
        product_category.trim(),
        scene_description?.trim() || null,
        price_overlay?.trim() || null
      ).then((imageUrl) => {
        console.log('✅ Background image generation completed:', imageUrl);
      }).catch((error) => {
        console.error('❌ Background image generation failed:', error);
      ...
      return { screen: 'COLLECT_INFO', data: { error_message: 'No data received.' } };
    }
  }
